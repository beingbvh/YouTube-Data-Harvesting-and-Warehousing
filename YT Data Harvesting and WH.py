...
if selected == "Home":
    ...
    st.markdown("## :violet[Skills take away From This Project] : Python scripting, Data Collection, MongoDB, Streamlit, API integration, Data Management using MongoDB and SQL")
    ...
    st.markdown("## :violet[Developed by] : BALAVIGNESH S S")
# ========================================   /     Data collection zone and Store data to MongoDB    /   =================================== #
...
if selected == "Data Zone":
    ...
    with tab1:
        ...
        st.write(
            '(**collects data** by using channel id and **stores it in the :orange[MongoDB] database**.)')
        ...
        if Get_data or st.session_state.Get_state:
            ...
            def get_video_data(youtube, video_ids):
                ...
                for video_id in video_ids:
                    try:
                        ...
                        st.write('You have exceeded your YouTube API quota.')
            ...
            client = pymongo.MongoClient('Your MongoDB server ID')
            ...
            st.write(f"Updated Channel id: {upload.upserted_id if upload.upserted_id else upload.modified_count}")
...
# ========================================   /     Data Migrate zone (Stored data to MySQL)    /   ========================================== #
    ...
    with tab2:
        ...
        st.write('''( **Migrates channel data from :blue[MongoDB] database to :green[MYSQL] database**)''')

        # Connect to the MongoDB server
        client = pymongo.MongoClient("Your MongoDB server")
        ...
        if Migrate or st.session_state_migrate_sql:
            ...
            engine = create_engine('mysql+mysqlconnector://user:password@host/youtube_db', echo=False)
...
if selected == "Query Zone":
    ...
    connect_for_question = pymysql.connect(host='hostname', user='username', password='Your password', db='youtube_db')
